...
class TxnProcessingUnit:
    ...
    async def update_token_account(self, wallet_address, wallet_token_account, block_time):
        try:
            ...
            if not result:
                ...
                # Commit the changes
                self.conn.commit()
                print(f"{color_style.CYAN}New token account added for wallet address: {str(wallet_address)}, token account: {wallet_token_account_str}",color_style.RESET)
    ...
    async def get_new_token_accounts(self,wallet_address: Pubkey):
        ...
        for token_account in all_token_accounts:
            ...
            if not account_exist:
                ...
                print("Account already exists")
        print(f"{color_style.GREEN}Added all token account of {wallet_address} to the database",color_style.RESET)
    ...
    async def transactionDetails(self , txn: GetTransactionResp,transaction_array:list):
          ...
          print(color_style.RED + "Length of inforamtion araay", len(information_array), color_style.RESET)
          ...
          if len(information_array)==2:
              ...
              try:
                ...
                if first_authority== str(self.wallet_address) and str(transfer_type) != str(WRAPPED_SOL_MINT):
                  ...
                  print(f"{color_style.GREEN}BUY {sol_spent} SOL {color_style.RESET} -FOR  {token_bought} TokenBought= {mint_address}")
                else:
                  self.update_sell(token_sold, txn_fee, block_time)
                  self.earned_sol += sol_sold
                  self.contract = mint_address
                  self.last_trade = block_time
                  print(f"{color_style.RED}SELL {int(token_sold)} Token {color_style.RESET} -FOR  {sol_sold} SOL TokenSold= {mint_address}")
              ...
              try:
                  ...
                  if str(jupyter_transaction['last']['mint']) != str(WRAPPED_SOL_MINT) and str(jupyter_transaction['last']['mint']) != "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v":
                      ...
                      print(color_style.YELLOW + "This is jupyter BUYYY", color_style.RESET)
                      ...
                      print(color_style.BLUE + "This is jupyter SELL", color_style.RESET)
                      from_wallet = jupyter_transaction['first']['authority']
                      to_wallet = jupyter_transaction['last']['authority']
    ...
    async def process_token_account(self, token_account: Pubkey,wallet_address_id: int):
        ...
        for signature in reversed(sig.value):
            if signature.err == None:
                ...
                print(color_style.RED,signature.signature,color_style.RESET)
                ...
                try:
                    if valid>0:
                        ...
                        print(color_style.CYAN,"Transaction Details Added",color_style.RESET)
                        valid=0
                    else:
                        pass
                except Exception as e:
                    print(e, signature.signature)
                    print(color_style.RED,"Error Adding Transaction Details",color_style.RESET)
                    continue
